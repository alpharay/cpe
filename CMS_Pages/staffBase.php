...
<html>
<head>
    <!--meta http-equiv="refresh" content="15"><this is to cause the page to refresh when inactive for a period to log out-->
    <style type ="text/css">
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function(){
            jQuery('#slideshow').fadeSlideShow({
                ...
                height:587, // height of your slideshow
...
div#copyright a:hover { color:#fff; }
    ...
    <body>        
        ...
        $picPresentVerifier= new PictureProcessing();
        ...
        //header("location: staffLogin.php?autoLgID=77");
       ...
       if($_SESSION['authentic']==true){ 
        ...
        $alumnusUser=$item->getByID($_SESSION['useRID']);           
        
        //loading the persons image
        echo "<img src = 'getTest.php?id={$alumnusUser[0]->getMemberID()}' height='100px' width='100px' style='' >";
        echo "&nbsp&nbsp&nbsp<BR><BR><form action='editPg.php' method='POST'><input type='submit' name='edit' value='EDIT PROFILE' style='width:100px; color:blue; cursor:pointer;'/>";        
        
        echo "<input type='hidden' name='facLevID1' value='{$alumnusUser[0]->getFacLevID()}'/>";

        echo "<input type='hidden' name='memID' value='{$alumnusUser[0]->getMemberID()}'/>";
        echo "<input type='hidden' name='firstName' value='{$alumnusUser[0]->getFirstName()}'/>";
        echo "<input type='hidden' name='surName' value='{$alumnusUser[0]->getSurName()}'/>";
        echo "<input type='hidden' name='otherName' value='{$alumnusUser[0]->getOtherNames()}'/>";
        echo "<input type='hidden' name='researchIn' value='{$alumnusUser[0]->getResearchInterest()}'/>";
        echo "<input type='hidden' name='memProfile' value='{$alumnusUser[0]->getMemProfile()}'/>";
        echo "<input type='hidden' name='emailAddr' value='{$alumnusUser[0]->getEmailAddress()}'/>";
         if($picPresentVerifier->personHasPic($alumnusUser[0]->getMemberID())==false)
        {
          echo "&nbsp&nbsp<input type='submit' name='UploadPic1' value='UPLOAD PICTURE' style='width:120px; color:blue; cursor:pointer;'/>";            
        }else{
             echo "&nbsp&nbsp<input type='submit' name='changePic1' value='CHANGE PICTURE' style='width:120px; color:blue; cursor:pointer;'/>";            
        }
        
        echo "&nbsp&nbsp<input type='submit' name='logout' value='LOGOUT' style='width:100px; color:blue; cursor:pointer;'/></form>";       

        echo "<br>";        
        echo "STAFF:&nbsp&nbsp&nbsp".$alumnusUser[0]->getFirstName()." ".$alumnusUser[0]->getOtherNames()." ".$alumnusUser[0]->getSurName()." "."<br><br>";                     
        echo "QUALIFICATION:&nbsp&nbsp&nbsp".$alumnusUser[0]->getQualification()."<br><br>";
        echo "EMAIL:&nbsp&nbsp&nbsp".$alumnusUser[0]->getEmailAddress()."<br><br>";
        echo "RESEARCH INTEREST:&nbsp&nbsp&nbsp".$alumnusUser[0]->getResearchInterest()."<br><br>";
        
        echo "&nbsp&nbsp<form method='POST' action='pdfLoadingPg.php'><input type='submit' name='logout' value='UPLOAD PDF DOCUMENT' style='width:200px; color:blue; cursor:pointer;'/></form><br/>";       

        echo "<a href='pwdChg.php'>CHANGE PASSWORD</a><br/><br/>";
        ?> 
