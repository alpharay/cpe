...
div#copyright a:hover { color:#fff; }
    ...
    <body>        
       ...
       if($_SESSION['authentic']==true){ 
        ...
        echo "&nbsp&nbsp&nbsp<BR><BR><form action='pwdChg.php' method='POST'>";
        echo "<input type='hidden' name='PwdChgrIndicator' value='yes'/>";
        echo "<br/><label '>Enter Original password:<label/><input type='password' name='pwdOrg' style='background-color:lightblue;'/>";
        echo "<br/><label style='margin-left:59px;'>New Password:<label/><input type='password' name='pwdChng' style='background-color:lightblue;'/>";
        echo "<br/><label style='margin-left:10px;'>Retype New Password:<label/><input type='password' name='pwdChng1' style='background-color:lightblue;'/>";
        echo "&nbsp&nbsp&nbsp<BR><BR><form action='editPg.php' method='POST'><input type='submit' name='edit' value='CHANGE PASSWORD' style='width:200px; color:blue; cursor:pointer;'/>";                
        ...
        echo "</form>";      
        echo "&nbsp&nbsp<a href='staffBase.php' style='color:green; font-size:15px;'/>back</a>";       
        $origPwd='yes';
        if(isset($_POST['pwdOrg']))$origPwd=$_POST['pwdOrg'];
        if($alumnusUser[0]->getPwdStatus()==1)$origPwd=$encryptor->encryptPassW2($origPwd);
        if(isset($origPwd)&&($origPwd==$alumnusUser[0]->getPassWord())){       
           
        $newUsr=new FacultyStaffOrMem();
        $newUsr->setMemberID($alumnusUser[0]->getMemberID());
        if(isset($_POST['pwdChng'])&&($_POST['pwdChng']==$_POST['pwdChng1'])){            
            $newPwd1=$encryptor->encryptPassW2($_POST['pwdChng']);
            
            $newUsr->setPassWord($newPwd1);
            $passWdChger=new FacultyStaffOrMemService();
            if($passWdChger->updatePwdByID($newUsr))echo "<br/>Password Successfully Changed";
            else{echo "<br/>change unsuccessful";}
            }else{echo "<br/>new Passwords did not much";}
        }
        elseif(isset($_POST['PwdChgrIndicator'])&&isset($origPwd)&&($origPwd!=$alumnusUser[0]->getPassWord())){echo "<br/>incorrect pass";}
        ?> 
